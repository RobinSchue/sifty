...
| **analyze()**            | The engine's one entry point — `({path, content}, {config, provider?, …}) → AnalyzeResult` (`{report, failures, aiError?, usage?}`), pure, no file system, no network | `src/engine/runner.ts`                            |
