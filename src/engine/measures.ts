...
function frontmatterLine(ctx: FileContext, field: string): number | undefined {
  const index = ctx.rawLines.findIndex((line) => new RegExp(`^\\s*${field}\\s*:`, "i").test(line));
...
const headingStructure: MeasureFn = ({ ctx, params }) => {
  ...
  const headings = ctx.body
    ...
    .map((item) => ({ line: item.line, level: item.match[1]?.length ?? 1 }));
  ...
  if (!allowSkips) {
    for (let i = 1; i < headings.length; i++) {
      ...
      if (current.level - previous.level > 1) {
        ...
        evidence.push({
          line: current.line,
          hint: `level jumps from h${previous.level} to h${current.level}`,
        });
      }
    }
  }
